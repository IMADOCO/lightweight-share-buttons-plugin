...
$lightweight_share_buttons_services      = array(
	'native'   => array( 'label' => __( 'Share', 'lightweight-share-buttons' ), 'url' => '' ),
	'copy'     => array( 'label' => __( 'Copy Link', 'lightweight-share-buttons' ), 'url' => '' ),
	'facebook' => array( 'label' => 'Facebook', 'url' => 'https://www.facebook.com/sharer/sharer.php?u=' . $lightweight_share_buttons_encoded_url ),
	'whatsapp' => array( 'label' => 'WhatsApp', 'url' => 'https://wa.me/?text=' . rawurlencode( trim( $lightweight_share_buttons_title . ' ' . $lightweight_share_buttons_text . ' ' . $lightweight_share_buttons_url ) ) ),
	'linkedin' => array( 'label' => 'LinkedIn', 'url' => 'https://www.linkedin.com/sharing/share-offsite/?url=' . $lightweight_share_buttons_encoded_url ),
	'email'    => array( 'label' => __( 'Email', 'lightweight-share-buttons' ), 'url' => 'mailto:?subject=' . $lightweight_share_buttons_encoded_title . '&body=' . $lightweight_share_buttons_email_body ),
	'telegram' => array( 'label' => 'Telegram', 'url' => 'https://t.me/share/url?url=' . $lightweight_share_buttons_encoded_url . '&text=' . rawurlencode( trim( $lightweight_share_buttons_title . ' ' . $lightweight_share_buttons_text ) ) ),
	'x'        => array( 'label' => 'X', 'url' => 'https://twitter.com/intent/tweet?url=' . $lightweight_share_buttons_encoded_url . '&text=' . rawurlencode( trim( $lightweight_share_buttons_title . ' ' . $lightweight_share_buttons_text ) ) ),
	'bluesky'  => array( 'label' => 'Bluesky', 'url' => 'https://bsky.app/intent/compose?text=' . rawurlencode( trim( $lightweight_share_buttons_title . ' ' . $lightweight_share_buttons_text . ' ' . $lightweight_share_buttons_url ) ) ),
	'threads'  => array( 'label' => 'Threads', 'url' => 'https://www.threads.net/intent/post?text=' . rawurlencode( trim( $lightweight_share_buttons_title . ' ' . $lightweight_share_buttons_text . ' ' . $lightweight_share_buttons_url ) ) ),
	'reddit'   => array( 'label' => 'Reddit', 'url' => 'https://www.reddit.com/submit?url=' . $lightweight_share_buttons_encoded_url . '&title=' . $lightweight_share_buttons_encoded_title ),
);
...
if ( ! empty( $attributes['useBrandColors'] ) ) { $lightweight_share_buttons_class_names[] = 'has-brand-colors'; }
if ( ! empty( $attributes['wrap'] ) ) { $lightweight_share_buttons_class_names[] = 'is-wrap'; }
$lightweight_share_buttons_wrapper_attributes = get_block_wrapper_attributes( array( 'class' => implode( ' ', $lightweight_share_buttons_class_names ), 'style' => '--lsb-gap:' . absint( $attributes['gap'] ) . 'px', 'data-url' => esc_url( $lightweight_share_buttons_url ), 'data-title' => esc_attr( $lightweight_share_buttons_title ), 'data-text' => esc_attr( $lightweight_share_buttons_text ), 'data-copied-label' => esc_attr__( 'Copied!', 'lightweight-share-buttons' ), 'data-copy-prompt' => esc_attr__( 'Copy this link:', 'lightweight-share-buttons' ) ) );
?>
<div <?php echo $lightweight_share_buttons_wrapper_attributes; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>>
	<?php if ( ! empty( $attributes['showHeading'] ) && '' !== trim( $attributes['heading'] ) ) : ?><p class="lsb-heading"><?php echo esc_html( $attributes['heading'] ); ?></p><?php endif; ?>
	<div class="lsb-buttons" role="group" aria-label="<?php esc_attr_e( 'Share this page', 'lightweight-share-buttons' ); ?>">
		<?php foreach ( $attributes['services'] as $lightweight_share_buttons_service ) : if ( ! isset( $lightweight_share_buttons_services[ $lightweight_share_buttons_service ] ) ) { continue; } $lightweight_share_buttons_item = $lightweight_share_buttons_services[ $lightweight_share_buttons_service ]; ?>
			<?php
			$lightweight_share_buttons_button_style = '';
			$lightweight_share_buttons_text_style   = '';
			if ( empty( $attributes['useBrandColors'] ) ) {
				$lightweight_share_buttons_background = sanitize_hex_color( $attributes['serviceColors'][ $lightweight_share_buttons_service ]['background'] ?? '' ) ?: '#000000';
				$lightweight_share_buttons_text_color = sanitize_hex_color( $attributes['serviceColors'][ $lightweight_share_buttons_service ]['text'] ?? '' ) ?: '#ffffff';
				if ( $lightweight_share_buttons_background ) { $lightweight_share_buttons_button_style = 'background-color:' . $lightweight_share_buttons_background . ';'; }
				if ( $lightweight_share_buttons_text_color ) { $lightweight_share_buttons_button_style .= 'color:' . $lightweight_share_buttons_text_color . ';'; $lightweight_share_buttons_text_style = ' style="color:' . esc_attr( $lightweight_share_buttons_text_color ) . ';"'; }
			}
